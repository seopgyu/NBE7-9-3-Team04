...
data class PostResponse(
    ...
    @field:Schema(description = "제목", example = "첫번째 게시물")
    val title: String,

    @field:Schema(description = "한 줄 소개", example = "사이드 프로젝트 할 팀원 구합니다")
    val introduction: String,

    @field:Schema(description = "내용", example = "백엔드 2명, 프론트엔드 2명 구합니다. 주제는")
    val content: String,

    @field:Schema(description = "마감일", example = "2025-10-25T17:00:00")
    val deadline: LocalDateTime,

    @field:Schema(description = "작성일", example = "2025-10-18T10:30:00")
    val createDate: LocalDateTime,

    @field:Schema(description = "수정일", example = "2025-10-18T10:50:00")
    val modifyDate: LocalDateTime,

    @field:Schema(description = "진행 상태", example = "ING", allowableValues = ["ING", "CLOSED"])
    val status: PostStatus,

    @field:Schema(description = "상단 고정 여부", example = "NOT_PINNED", allowableValues = ["PINNED", "NOT_PINNED"])
    val pinStatus: PinStatus,

    @field:Schema(description = "모집 인원", example = "4")
    val recruitCount: Int,

    @field:Schema(description = "작성자의 닉네임", example = "개발자A")
    val nickName: String,

    @field:Schema(description = "카테고리 타입", example = "PROJECT")
    val categoryType: PostCategoryType,

    @field:Schema(description = "내 게시글 여부", example = "true")
    val isMine: Boolean
) {
    companion object {
