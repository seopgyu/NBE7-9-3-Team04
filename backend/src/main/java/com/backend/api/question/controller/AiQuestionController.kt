...
class AiQuestionController(
    ...
    fun readAllAiQuestion(): ApiResponse<AiQuestionReadAllResponse> {
        val userId = rq.getUser().id
        val responses = aiQuestionService.readAllAiQuestion(userId)
    ...
    ): ApiResponse<PortfolioListReadResponse> {
        val userId = rq.getUser().id
        val responses = aiQuestionService.readAiQuestion(userId, groupId)
